                ...
                <div class="bg-white rounded-xl shadow-sm overflow-hidden">
                    <div class="divide-y divide-gray-100">
                        @foreach($cartItems as $item)
                            <div class="p-6 flex gap-4 hover:bg-gray-50 transition cart-item" data-price="{{ $item->product->price }}">
                                {{-- Product image --}}
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-start justify-between gap-4 mb-3">
                                        <div>
                                            <a href="{{ route('products.show', $item->product) }}" class="text-base font-semibold text-gray-900 hover:text-blue-600 truncate">
                                                {{ $item->product->name }}
                                            </a>
                                    ...
                                    <div class="flex items-center justify-between gap-4">
                                        <form action="{{ route('cart.update', $item) }}" method="POST" class="flex items-center gap-2">
                                            @csrf
                                            @method('PATCH')
                                            <button type="button" onclick="changeQty(this, -1)" class="px-2 py-1 bg-gray-100 rounded hover:bg-gray-200">−</button>
                                            <input type="number" name="quantity" value="{{ $item->quantity }}" min="1" max="{{ $item->product->stock }}" class="w-12 text-center border border-gray-300 rounded py-1 qty-input" onchange="clampQty(this)">
                                            <button type="button" onclick="changeQty(this, 1)" class="px-2 py-1 bg-gray-100 rounded hover:bg-gray-200">+</button>
                                        </form>
                                        <div class="text-right">
                                            <p class="text-sm text-gray-500">Rp {{ number_format($item->product->price, 0, ',', '.') }}</p>
                                            <p class="text-lg font-semibold text-gray-900" data-role="line-total">
            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                    ...
                    <div class="flex justify-between mb-6">
                        ...
                        <span id="summary-total" class="text-2xl font-bold text-gray-900">Rp {{ number_format($total, 0, ',', '.') }}</span>
                    </div>

                    {{-- Checkout button --}}
                    <a href="{{ route('cart.checkout.show') }}" class="w-full bg-black text-white py-3 rounded-lg hover:bg-gray-800 transition font-medium text-center block mb-3">
                        Proceed to Checkout
                    </a>

                    {{-- Continue shopping --}}
                    <a href="{{ route('products.index') }}" class="w-full border border-gray-300 text-gray-700 py-2.5 rounded-lg hover:bg-gray-50 transition font-medium text-center block text-sm">
                        Continue Shopping
                    </a>
...
<script>
    ...
    // Persist the new quantity to the server (PATCH cart.update) without reloading
    function persistQty(input) {
        const form = input.closest('form');
        fetch(form.action, {
            method: 'POST', // Laravel reads _method=PATCH from the form body
    ...
    function changeQty(btn, delta) {
        ...
        const val = Math.max(1, Math.min(max, current + delta));
        if (val === current) return; // already at the min/max bound
        input.value = val;
        recalcTotals();
        persistQty(input);
    }
